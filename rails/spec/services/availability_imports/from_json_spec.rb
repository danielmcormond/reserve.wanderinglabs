...
RSpec.describe AvailabilityImports::FromJson do
  ...
  describe '#perform' do
    ...
    it 'creates the avail_at range successfully' do
      from_json.perform
      expect(import.availabilities.where(site_id: 2).first.avail_at.first).to eq(Date.strptime('11/24/2017', '%m/%d/%Y'))
      expect(import.availabilities.where(site_id: 2).first.avail_at.last).to eq(Date.strptime('11/26/2017', '%m/%d/%Y'))
    end

    it 'creates the avail_at range successfully with one date' do
      from_json.perform
      expect(import.availabilities.where(site_id: 3).first.avail_at.first).to eq(Date.strptime('11/25/2017', '%m/%d/%Y'))
      expect(import.availabilities.where(site_id: 3).first.avail_at.last).to eq(Date.strptime('11/25/2017', '%m/%d/%Y'))
    end
    ...
    context 'second import with no availabilities' do
      let(:import2) { FactoryGirl.create(:availability_import, facility: import.facility) }
      let(:from_json2) { AvailabilityImports::FromJson.new(import2) }
