...
export function fetchAvailabilitySites() {
  ...
  return function (dispatch) {
    ...
    reserveApi({
      method: 'get',
      url: `/availability_requests/${uuid}/sites.json`
    })
      .then(response => {
        dispatch({ type: 'FETCH_AR_SITES_FULFILLED', payload: camelcaseKeys(response.data, { deep: true }) })
      })
      .catch(err => {
        dispatch({ type: 'FETCH_AR_SITES_REJECTED', payload: err })
      })
  }
}
