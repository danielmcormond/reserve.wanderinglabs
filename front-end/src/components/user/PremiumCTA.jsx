...
const PremiumCTA = () => {
  ...
  const amountChecks = amounts.map(checkAmount => (
    <label key={`amount_${amount}`} class="flex flex-inline mr-6 mt-3 mb-3" onClick={() => setAmount(checkAmount)}>
      <div class="bg-white border-2 rounded border-gray-400 w-6 h-6 flex flex-shrink-0 justify-center items-center mr-2 mt-2 focus-within:border-blue-500">
        <input type="checkbox" class="opacity-0 absolute" />
        <svg class={`fill-current ${amount === checkAmount ? '' : 'hidden'} w-4 h-4 text-green-500 pointer-events-none`} viewBox="0 0 20 20">
          <path d="M0 11l2-2 5 5L18 3l2 2L7 18z" />
        </svg>
      </div>
      <div class="select-none text-2xl">${checkAmount}</div>
    </label>
  ))
