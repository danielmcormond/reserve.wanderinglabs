    ...
    <div className="flex items-stretch">
      {siteTypeOptionsReduced.map(option => (
        <div className="flex w-1/5" key={option.text}>
          ...
          >
            <FontAwesomeIcon icon={option.icon} className="text-xl sm:text-3xl md:text-6xl" />
            <h3 className="mt-1 md:mt-2 font-semibold tracking-tighter md:tracking-tight text-sm">{option.text}</h3>
          </div>
        </div>
      ))}
    </div>
